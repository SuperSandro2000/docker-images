...
RUN apt-get update \
  ...
  && curl -Lso Lavalink.jar "$(curl -s https://api.github.com/repos/Cog-Creators/Red-DiscordBot/releases?access_token=$${GITHUB_TOKEN} | jq -r '.[0].assets | .[0].browser_download_url')" \
