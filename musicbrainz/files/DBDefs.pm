...
sub GPG_MISSING_SIGNATURE_MODE { $ENV{'GPG_MISSING_SIGNATURE_MODE'} || 'PASS' }
...
# sub GPG_SIGN_KEY { "" }
# sub GPG_ENCRYPT_KEY { "" }
sub GPG_SIGN_KEY { $ENV{'GPG_SIGN_KEY'} || '' }
sub GPG_ENCRYPT_KEY { $ENV{'GPG_ENCRYPT_KEY'} || '' }
...
sub WEB_SERVER { $ENV{'WEB_SERVER'} || 'musicbrainz.example.com' }
...
sub WEB_SERVER_SSL { $ENV{'WEB_SERVER_SSL'} || 'localhost' }
sub SEARCH_SERVER { $ENV{'SEARCH_SERVER'} || 'search.musicbrainz.org' }
sub SEARCH_ENGINE { $ENV{'SEARCH_ENGINE'} || 'LUCENE' }
...
sub SMTP_SERVER { $ENV{'SMTP_SERVER'} || 'localhost' }
...
sub random_token {
	use String::Random;
	return String::Random->new->randregex();
}

sub SMTP_SECRET_CHECKSUM { $ENV{'SMTP_SECRET_CHECKSUM'} || random_token }
sub EMAIL_VERIFICATION_TIMEOUT { 86400 } # one week
...
sub DB_STAGING_SERVER { $ENV{'DB_STAGING_SERVER'} || 0 }
...
sub DB_STAGING_SERVER_DESCRIPTION { $ENV{'DB_STAGING_SERVER_DESCRIPTION'} || '' }
...
sub DB_STAGING_SERVER_SANITIZED { $ENV{'DB_STAGING_SERVER_SANITIZED'} || 1 }
...
sub SSL_REDIRECTS_ENABLED { $ENV{'SSL_REDIRECTS_ENABLED'} || 0 }
...
sub WIKITRANS_SERVER_API { $ENV{'WIKITRANS_SERVER_API'} || 'wiki.musicbrainz.org/api.php' }
...
sub GOOGLE_CUSTOM_SEARCH { $ENV{'GOOGLE_CUSTOM_SEARCH'} || '006539527923176875863:xsv3chs2ovc' }
...
sub PLUGIN_CACHE_OPTIONS {
    ...
    return {
        class => 'MusicBrainz::Server::CacheWrapper::Redis',
        server => $ENV{'REDIS_SERVER'} || 'redis:6379',
        namespace => $self->CACHE_NAMESPACE . 'Catalyst:',
    };
}
...
sub ENTITY_CACHE_TTL { $ENV{'ENTITY_CACHE_TTL'} || 0 }
...
sub SESSION_STORE { "Session::Store::MusicBrainz" }
sub SESSION_STORE_ARGS { return {} }
sub SESSION_EXPIRE { return $ENV{'SESSION_EXPIRE'} || 24 * 60 * 60; }
...
sub DATASTORE_REDIS_ARGS {
    my $self = shift;
    return {
        database => $ENV{'REDIS_ID'} || 0,
        namespace => $self->CACHE_NAMESPACE,
        server => $ENV{'REDIS_SERVER'} || 'redis:6379',
        test_database => $ENV{'REDIS_ID_TEST'} || 1,
    };
}
...
sub WEB_SESSION_SECONDS_TO_LIVE { $ENV{'WEB_SESSION_SECONDS_TO_LIVE'} || 24 * 60 * 60 }

# The cookie name to use
# sub SESSION_COOKIE { "AF_SID" }
sub SESSION_COOKIE { $ENV{'SESSION_COOKIE'} || 'AF_SID' }

# The domain into which the session cookie is written
# sub SESSION_DOMAIN { undef }
sub SESSION_DOMAIN { $ENV{'WEB_SERVER'} || undef }
...
# sub DB_READ_ONLY { 0 }
sub DB_READ_ONLY { $ENV{'DB_READ_ONLY'} || 0 }
...
# EOF
sub DB_READ_ONLY_MESSAGE { $ENV{'DB_READ_ONLY_MESSAGE'} || <<EOF }
This server is temporarily in read-only mode
for database maintainance.
EOF
...
sub ANNOTATION_LOCK_TIME { $ENV{'ANNOTATION_LOCK_TIME'} || 15 * 60 }
...
# sub AWS_PRIVATE { '' }
# sub AWS_PUBLIC { '' }
sub AWS_PRIVATE { $ENV{'AWS_PRIVATE'} || '' }
sub AWS_PUBLIC { $ENV{'AWS_PUBLIC'} || '' }

# sub AMAZON_ASSOCIATE_TAG { '' }
sub AMAZON_ASSOCIATE_TAG { $ENV{'AMAZON_ASSOCIATE_TAG'} || '' }
...
sub RECAPTCHA_PUBLIC_KEY { $ENV{'RECAPTCHA_PUBLIC_KEY'} || return undef }
sub RECAPTCHA_PRIVATE_KEY { $ENV{'RECAPTCHA_PRIVATE_KEY'} || return undef }
...
sub COVER_ART_ARCHIVE_ACCESS_KEY { $ENV{'COVER_ART_ARCHIVE_ACCESS_KEY'} || undef };
sub COVER_ART_ARCHIVE_SECRET_KEY { $ENV{'COVER_ART_ARCHIVE_SECRET_KEY'} || undef };
sub COVER_ART_ARCHIVE_UPLOAD_PREFIXER { shift; sprintf("//%s.s3.us.archive.org/", shift) };
sub COVER_ART_ARCHIVE_DOWNLOAD_PREFIX { "//coverartarchive.org" };
...
# sub USE_ETAGS { 1 }
sub USE_ETAGS { $ENV{'USE_ETAGS'} || 1 }

# sub CATALYST_DEBUG { 1 }
sub CATALYST_DEBUG { $ENV{'CATALYST_DEBUG'} || 0 }
...
sub DEVELOPMENT_SERVER { $ENV{'DEVELOPMENT_SERVER'} || 0 }
...
sub SENTRY_DSN { $ENV{'SENTRY_DSN'} || undef }
sub SENTRY_DSN_PUBLIC { $ENV{'SENTRY_DSN_PUBLIC'} || undef }
...
sub HTML_VALIDATOR { $ENV{'HTML_VALIDATOR'} || 'http://validator.w3.org/nu/?out=json' }
...
sub PROFILE_WEB_SERVICE { $ENV{'PROFILE_WEB_SERVICE'} || 0 }

# Log if a request in / (not /ws) takes more than x seconds
# sub PROFILE_SITE { 0 }
sub PROFILE_SITE { $ENV{'PROFILE_SITE'} || 0 }
